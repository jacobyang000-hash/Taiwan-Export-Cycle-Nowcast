"""Load MOEA industrial production index from a manually downloaded Excel file."""
...
def load_industrial_production():
    df = pd.read_excel(RAW / "MOEA_Industrial_Production_Raw.xlsx", skiprows=1, header=None)

    # Keep only the first 3 columns — the 4th is blank junk in this file
    df = df.iloc[:, :3]
    df.columns = ["year_raw", "month_raw", "industrial_production"]
    ...
    df["industrial_production"] = pd.to_numeric(df["industrial_production"], errors="coerce")

    tidy = df[["date", "industrial_production"]].dropna(subset=["industrial_production"])
    tidy = tidy.melt(id_vars="date", var_name="series_id", value_name="value")
    return tidy.sort_values(["series_id", "date"])


if __name__ == "__main__":
    out = load_industrial_production()
    print(out.head())
    print(out.tail())
    print(out.shape)
    PROCESSED.mkdir(parents=True, exist_ok=True)
    out.to_parquet(PROCESSED / "moea_ip.parquet", index=False)
    print("saved")